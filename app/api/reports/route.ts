...
async function buildDailyStats(
  ...
  const [
    ...
    prisma.task.findMany({
      where: { userId, status: "DONE", updatedAt: { gte: start, lte: end } },
      select: { updatedAt: true },
    }),
    prisma.pointLog.findMany({
      where: { userId, createdAt: { gte: start, lte: end }, amount: { gt: 0 } },
...
export async function GET(request: Request) {
  ...
  const result: ReportStats = {
    ...
    comparison: {
      ...
      moodAverage:
        currentSummary.moodAverage !== null && previousSummary.moodAverage !== null
          ? Number(
